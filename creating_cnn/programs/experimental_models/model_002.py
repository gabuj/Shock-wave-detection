...
class UNet(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(512, 256, kernel_size=3, stride=1, padding=1),  # (Keeps same size)
            nn.ReLU(inplace=True),

                # Upsample: 256 → 128
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),  # (Doubles spatial size)
            nn.ReLU(inplace=True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # (Refinement conv)
            nn.ReLU(inplace=True),

                # Upsample: 128 → 64
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),  # (Extra conv layer)
            nn.ReLU(inplace=True),

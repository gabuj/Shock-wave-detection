...
class UNetWithSkipConnections(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # Encoder: utilizzo di VGG16 con punti di estrazione di feature
        ...
        # Modifica per input grayscale
        self.enc1[0] = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1)
        
        # Decoder con skip connections
        self.dec5 = self._decoder_block(512, 512, 256)
        self.dec4 = self._decoder_block(256, 256, 128)  # Rimosso canali extra per skip connection
        self.dec3 = self._decoder_block(128, 128, 64)   # Rimosso canali extra per skip connection
        self.dec2 = self._decoder_block(64, 64, 32)     # Rimosso canali extra per skip connection
        self.dec1 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=3, padding=1),  # Rimosso canali extra per skip connection
    ...
    def forward(self, x):
        ...
        # Decoder con resize delle feature per assicurare dimensioni corrette
        d5 = self.dec5(e5)
        ...
        # Assicuriamoci che l'output abbia la stessa dimensione dell'input
        if d1.shape[2:] != x.shape[2:]:
            d1 = F.interpolate(d1, size=x.shape[2:], mode='bilinear', align_corners=False)
        
        return d1

# Per mantenere compatibilità con il codice esistente, creiamo una classe UNet che è un wrapper di UNetWithSkipConnections
class UNet(UNetWithSkipConnections):
    def __init__(self, pretrained=True):
        super(UNet, self).__init__(pretrained=pretrained)

# Funzione di loss combinata
def combined_loss(pred, target, bce_weight=0.5, fp_weight=2.0, gamma=2.0):
    ...
    weights = weights + (fp_weight - 1.0) * (1 - target)  # More weight where target=0
    ...
    # Modified Dice Loss with emphasis on false positives
    smooth = 1.0
    ...
    union = pred_flat.sum() + target_flat.sum() + (fp_weight - 1.0) * false_positives
